...
## Problem 13: One Hardcoded Ingestion Source
*   **The Issue:** The pipeline could only ever ingest the single M365 Copilot digest. `run-digest.sh` hardcoded that one source end-to-end (paths, prompt, archive folder, notification text), and `fetch_digest.py` carried the reusable Graph/markdown/ledger machinery inline, so a second source (e.g. the personal inbox) meant copy-pasting the whole orchestrator and fetcher.
*   **The Fix (Ingestion Framework):**
    *   Extracted the shared Graph helpers (`get_access_token`, `graph_get`, `html_to_markdown`, `load_ledger`, `resolve_folder_id`, `env`) out of `fetch_digest.py` into `scripts/graph_mail.py`. `fetch_digest.py` now imports them; its behavior is byte-identical.
    *   Added `scripts/run-ingest.sh <source>`, the generic orchestrator distilled from `run-digest.sh`: shared vault lock (sources serialize on `/work/ingest.lock`), pre-run snapshot, fetch step (unchanged 0/20/30 exit-code contract), vault-index precompute, agent run, dry-run `proposed.md` verification, raw archive, filing report, deferred ledger (and now watermark) commit, notify, prune. Per-source values come from env with a `${PREFIX}_*` convention (e.g. `PERSONAL_MAIL_*`), including a per-source `DRY_RUN` (a new source defaults to dry-run and does NOT inherit the global `DRY_RUN`, so taking the digest live never silently takes a new source live too) and optional per-source `LLM_MODEL`/`LLM_BASE_URL`/`LLM_API_KEY`.
    *   `run-digest.sh` is now a thin wrapper delegating to `run-ingest.sh digest`, keeping the existing cron entry, docs, ledger, and `Raw Digests/` archive untouched.
    *   *Note for future AI agents:* adding a source is a `case` entry in `run-ingest.sh` + a fetcher (staging markdown + `pending_ids.txt`) + a prompt. Per-source agent logs are `agent.<source>.<date>.json` (was `agent.<date>.json`) to avoid same-day collisions; the `agent.*.json` prune glob still covers them.

## Problem 14: Whole-Inbox Personal Triage
*   **The Issue:** The digest flow only ever saw one curated work-summary email. There was no way to capture the signal buried in the rest of the inbox (bookings, personal correspondence, finance/health/legal events) without also drowning the vault in marketing, OTPs, and receipts.
*   **The Fix (Triage source):** Added the `personal` source: `scripts/fetch_inbox.py` + `prompt_personal_email.txt` (live) and `prompt_personal_email_dry_run.txt` (proposes to `proposed.md`).
    *   **Start-from-now watermark:** first run records a watermark = now and exits 20 (files nothing historical). Later runs fetch Inbox messages `receivedDateTime ge watermark`, oldest-first, capped at `PERSONAL_MAIL_MAX_PER_RUN` (default 25) so a backlog drains across nights without blowing the context budget. Bodies are capped at ~12k chars each. The watermark advances only after a successful filing run (staged as `pending_watermark.txt`, committed by `run-ingest.sh` alongside the ledger), mirroring the deferred-ledger pattern.
    *   **Triage + audit:** the prompt adds a triage layer (skip marketing/newsletters/OTPs/automated notifications/routine receipts; file correspondence/plans/bookings/finance/health/legal/commitments) on top of the digest's durable rules (lossless, granularity, `YYYY-MM-DD (MMM)`, message-ID idempotency markers). The `finish` tool gained `skipped`/`skipped_details`, and `filing_report.py` renders them to both the ntfy alert and `Raw Email/Filing Log.md` as sub-bullets — the skip judgment is audited, not trusted blind.
    *   Scheduled nightly at 22:30 (offset an hour after the digest; they share the vault lock anyway).
