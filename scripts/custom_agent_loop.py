...
def write_file(path: str, content: str) -> str:
    ...
    if DRY_RUN and not path.endswith("proposed.md"):
...
def main():
    ...
    messages = [
        {"role": "system", "content": "You are an autonomous file-management agent for an Obsidian vault."},
        {"role": "user", "content": prompt}
    ]

    print(f"Starting custom agent loop with model {model} (DRY_RUN={DRY_RUN})...", file=sys.stderr)
    
    total_cost_usd = 0.0  # Placeholder, as OpenAI SDK doesn't natively return cost easily per token across all providers
    ...
    while loop_count < max_loops:
        ...
        for tool_call in msg.tool_calls:
            ...
            if func_name == "finish":
                # Emulate the JSON output for run-digest.sh or profile_vault.py to parse
                out = args
                if "total_cost_usd" not in out:
                    out["total_cost_usd"] = 0.0 # Unknown for generic endpoints
    ...
    print(json.dumps({"error": "Exceeded maximum tool call loops", "total_cost_usd": 0}))
    sys.exit(1)
